...
C   
      ...
      REAL(8) :: Tg, Tref, T0, Tstar
      ! differenr temperature parameters	
  	  !
	  ! some related temperature-dependent co-efficient
	  !
      REAL(8):: m_et,b_et                      
      ! temperature-dependent co-efficient for et  
      REAL(8):: m_lamda, b_lamda               
      ! temperature-dependent co-efficient for lamda
      REAL(8):: m_E1,b_E1
      ! temperature-dependent co-efficient for E1
      REAL(8):: m_deltaH, b_deltaH
      ! temperature-dependent co-efficient for deltaH
      REAL(8):: m_Astar, b_Astar
      ! temperature-dependent co-efficient for Astar
      REAL(8):: m_vstar, b_vstar
      ! temperature-dependent co-efficient for vstar
      REAL(8):: m_mu, b_mu
      ! temperature-dependent co-efficient for mu
      REAL(8):: m_hsoft, b_hsoft
      ! temperature-dependent co-efficient for hsoft
      REAL(8):: m_Dinf, b_Dinf
      ! temperature-dependent co-efficient for Dinf
      REAL(8):: m_H, b_H
      ! temperature-dependent co-efficient for H
      REAL(8):: m_youngs_first,m_youngs_second
      REAL(8)::	b_youngs_first,b_youngs_second 	
      ! temperature-dependent co-efficient for youngs	  
C     
      INTEGER iter,i,j
      ...
      parameter(m_et = -0.1742D0,     b_et= 0.1759D0,
     +          m_lamda= 0.5314D0,    b_lamda= 0.2634D0,
     +          m_E1= -155.95D0,      b_E1= 275.85D0,
     +          m_deltaH= -0.8505D0,  b_deltaH= 2.5166D0,  
     +          m_Astar= 104578.D0,   b_Astar= 21172.D0,  
     +          m_mu= 0.3173D0,       b_mu= 0.0057D0,
     +          m_hsoft= -6.2129D0,   b_hsoft= 13.23D0,
     +         	m_Dinf= -0.6586D0,    b_Dinf= 1.8128D0,
     +   	    m_H= 45.839D0,        b_H= 36.076D0,
     +          m_youngs_first =28.722D0, b_youngs_first= 1372.6D0,
     +  	    m_youngs_second =1404.5D0, b_youngs_second= 792.18D0)  
      !                                                    Get arguments 
      !================================================================= 
C      youngs = props(1)       ! youngs modulus 
      poisson = props(1)      ! poisson ratio 
      T = props(2)            ! temperature 
      suppress = props(3)     ! atmospheric pressure 
      m_vstar  = props(4)
      b_vstar  = props(5)	  
C       et = props(5)           ! gt, for calculating g1
C       lamda = props(6)        ! lamda  
C       E1 = props(7)           ! G1
C       deltaH = props(8)       ! enthalpy
C       Astar = props(9)        ! A0
C       vstar = props(10)       !  v* shear volume 
C       mu = props(11)          ! pressure co-efficient
C       hsoft = props(12)       ! slope of softening
C       Dinf = props(13)        ! saturation volume of softening parameter
C       H = props(14)           ! slope of plasticity curve    

      Tref      =  296.15d0
	  Tg        =  420.63d0
      Tstar     =  (Tg-T)/(Tg-Tref)
	  
      if (T .le. 368.15d0) then
	  
	      youngs    =  m_youngs_first * Tstar     + b_youngs_first
		  
      else
	      youngs    =  m_youngs_second * Tstar     + b_youngs_second
		  
      end if 	      
      et        =  m_et     * Tstar + b_et 	  
      lamda     =  m_lamda  * Tstar + b_lamda
      E1        =  m_E1     * Tstar + b_E1
      deltaH    = (m_deltaH * Tstar + b_deltaH)*1.0E8
      Astar     = (m_Astar  * Tstar + b_Astar)*1.0E-30	 
      vstar     = (m_vstar  * Tstar + b_vstar)*1.0E6
      mu        =  m_mu     * Tstar + b_mu
      hsoft     =  m_hsoft  * Tstar + b_hsoft
      Dinf      =  m_Dinf   * Tstar + b_Dinf
      H         =  m_H      * Tstar + b_H	  
      ...
      kmod1 = youngs /(R3 *(R1 - R2*poisson))
	  
	  gt = et*(R2 *(R1 + poisson))        !delft Phd thesis 5.69
	  kt = et*(R3 *(R1 - R2*poisson))
	  
      G1 = E1 /(R2 *(R1 + poisson)) 
      K1  = E1/(R3 *(R1 - R2*poisson))
